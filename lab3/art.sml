 ...
 *)
fun for (lo, hi, f) = let val i = ref lo in 
  while !i <= hi do (f (!i) ; i := !i+1) end;
...
(* build: int * RandomGenerator -> Expr 
   ...
   the expression.
*)
fun build (d, rand) = let
  (* deeper : 'a -> Expr, helper function to generate a shallower subexpression,
   * because clearly we do that a lot *)
  fun deeper _ = build (d-1, rand)
  val choices = [
    (* give half the chance of buildX/Y so they don't appear too frequently *)
    fn _ => if rand (0,1) = 0 then buildX () else buildY (),
    fn _ => buildSine (deeper ()),
    fn _ => buildCosine (deeper ()),
    fn _ => buildAverage (deeper (), deeper ()),
    fn _ => buildTimes (deeper (), deeper ()),
    fn _ => buildFrob (deeper (), deeper (), deeper ()),
    fn _ => buildPow (deeper (), deeper ()),
    fn _ => buildGlub (deeper (), deeper ()) ]
(* guaranteed not to go out of bounds but only because we know how many elements
 * are in choices *)
in List.nth (choices, if d * rand (0,8) = 0 then 0 else rand (1, 7)) () end;
...
*)
fun makeRand seeds = let val generator = Random.rand seeds in
  fn range => Random.randRange range generator end;
...
for (2, 5, fn x => print (exprToString (build (10, makeRand (80, x))) ^ "\n"));
(*emitGrayscale (eval sampleExpr, 150);*)
doRandomColor (6, 18, 130);
